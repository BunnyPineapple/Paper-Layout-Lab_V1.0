...
/* ═══════════════════════════════════════════════════════════════
   PAPER LAYOUT LAB — v0.9
   
   PRINT / EXPORT ARCHITECTURE:
   
   DOM structure:
   ┌─ .print-layout (hidden on screen, shown on print)
   │  └─ <Page forPrint={true} />  ← transparent bg, lines only
   │     ↑ printRef points here for PNG export
   │
   └─ .screen-preview (shown on screen, hidden on print)
      └─ <Page forPrint={false} /> ← paper bg, decorations
         ↑ displayed to user
   
   Print: window.print() → @media print hides screen-preview,
          shows print-layout. @page size = selected notebook mm.
   
   PNG:   Serialize printRef SVG → canvas → transparent PNG.
          Always reads CURRENT state because printRef is always
          rendered (just display:none on screen).
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   ...
   forPrint=false → Screen preview with paper background (#faf7f2)
   forPrint=true  → Print/export: TRANSPARENT, no background rect
   
   Size: width/height in SVG = mm × scale(3.5)
   For print @page: uses actual mm from SD[sz]
   ═══════════════════════════════════════════════════════════════ */
const Page = ({ sz, ly, lsp, ttl, sub, img, lc, pt, fid, phr, ckc, mh, mv, cs, colBlank = false, forPrint = false, lineOp = 1, dotSize = 1 }) => {
  ...
  const pat = (sy, ey, sx = xl, ex = pw - xr, p = "") => {
    ...
    if (pt === "lined") {
      ...
      const aR = sx + cols * ls, aB = sy + rows * ls;
      for (let row = 0; row <= rows; row++) r.push(<line key={p + "H" + i++} x1={sx} y1={sy + row * ls} x2={aR} y2={sy + row * ls} stroke={lc} strokeWidth="0.8" opacity={lineOp}/>);
      let j = 0;
      for (let col = 0; col <= cols; col++) r.push(<line key={p + "V" + j++} x1={sx + col * ls} y1={sy} x2={sx + col * ls} y2={aB} stroke={lc} strokeWidth="0.8" opacity={lineOp}/>);
    } else {
      const cols = Math.round((ex - sx) / ls);
      const rows = Math.round((ey - sy) / ls);
  ...
  if (ly === "A") {
    ...
    content = <>
      <rect x={xl} y={ct} width={pw - xl - xr} height={ih} fill="none" stroke={lc} strokeWidth="1" strokeDasharray="5 4" rx="3" opacity={lineOp}/>
      {img && <image href={img} x={xl} y={ct} width={pw - xl - xr} height={ih} preserveAspectRatio="xMidYMid slice"/>}
    ...
    content = <>
      ...
      <line x1={dv} y1={ls2} x2={dv} y2={cb} stroke={lc} strokeWidth="1" opacity={lineOp}/>
    ...
    content = <>
      ...
      {Array.from({ length: ckc }, (_, i) => { const y = cs2 + i * sp; return <g key={"CK" + i}><rect x={xl} y={y + 2} width="12" height="12" rx="2.5" fill="none" stroke={lc} strokeWidth="1" opacity={lineOp}/><line x1={xl + 20} y1={y + 14} x2={pw - xr} y2={y + 14} stroke={lc} strokeWidth=".7" opacity={lineOp}/></g>; })}
    </>;
  }
...
export default function App() {
  ...
  const printRef = useRef(null);  // Points to .print-layout div containing forPrint SVG
  ...
  /* ─── PNG EXPORT ───
     Reads from printRef (the forPrint=true SVG, transparent bg).
     Renders at 3x resolution for print quality.
     Canvas background is transparent (clearRect, not fillRect). */
  const doExportPNG = useCallback(() => {
    ...
    const si = SD[sz];
    const sc = 3.5;
    const canvasW = si.w * sc * 3, canvasH = si.h * sc * 3;
    ...
    const im = new Image();
    im.onload = () => {
      ctx.clearRect(0, 0, canvasW, canvasH); // transparent
      ctx.drawImage(im, 0, 0, si.w * sc, si.h * sc);
      setExpImg(canvas.toDataURL("image/png"));
    };
    im.src = "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(str)));
  }, [sz]);

  /* ─── PRINT ───
     Calls window.print(). The @media print CSS:
     - Hides .screen-preview
     - Shows .print-layout (transparent SVG)
     - Sets @page size to the selected notebook's mm dimensions */
  const doPrint = useCallback(() => {
    window.print();
  }, []);

  // Dynamic @page size based on selected notebook
  const pageSizeMM = sz ? `${SD[sz].w}mm ${SD[sz].h}mm` : "auto";

  /* ═══════════════════════════════════════
     PRINT CSS — generated dynamically so
     @page size matches selected notebook
     ═══════════════════════════════════════ */
  const printCSS = `
    .print-layout {
      display: none;
  ...
  if (v === "desk") {
    ...
    return (
      <div className="screen-preview" style={{ minHeight: "100vh", background: "linear-gradient(180deg,#f5f0e8,#ede6da)", fontFamily: "'Caveat',cursive", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: "780px", margin: "20px auto 0", padding: "0 16px" }}>
          <div style={{
        ...
        <div style={{ textAlign: "center", padding: "18px", fontSize: "12px", color: "#b8a99550" }}>Paper Layout Lab · v0.9</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,#f5f0e8,#ede6da)", fontFamily: "'Caveat',cursive" }}>
      ...
      {/* ══════════════════════════════════════════════════════
          ...
          This is the DOM node used for:
          1. Print (@media print shows this, hides everything else)
          2. PNG export (printRef reads SVG from here)
          
          forPrint=true → transparent background, no paper color
          ══════════════════════════════════════════════════════ */}
      <div className="print-layout" ref={printRef}>
        <Page {...pp} forPrint={true} />
      </div>

      {/* ══════════════════════════════════════════════════════
          SCREEN PREVIEW — all UI, controls, decorated preview
          Hidden when printing via @media print
          ══════════════════════════════════════════════════════ */}
      <div className="screen-preview">
        ...
        <div style={{ display: "flex", maxWidth: "1100px", margin: "0 auto", padding: "14px", gap: "18px", flexWrap: "wrap", justifyContent: "center" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", minWidth: "320px" }}>
            ...
            <div style={{ background: "#e8dfd420", padding: "18px", borderRadius: "8px", display: "inline-block", boxShadow: "0 2px 20px #5c504715" }}>
            ...
            <div style={{ marginTop: "12px", fontSize: "12px", color: "#b8a99540" }}>Paper Layout Lab · v0.9</div>
          </div>
        </div>
      </div>
    </div>
